...
console.log(first.attributes); // NamedNodeMap{0: class, 1: id, 2: some, class: class, id: id, some: some, length: 3}

//? 기본 for문
for (let i = 0; i < first.attributes; i++) {
  console.log(first.attributes[i]);
}

//? iterator 한 속성이므로 사용 가능, 배열과 객체를 순환해야 할 때는 for of 가 더 낫다.
for (let key of first.attributes) {
  //   console.log(key);
}
